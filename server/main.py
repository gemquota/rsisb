"""FastAPI server for RSIS telemetry and system control.

Provides REST endpoints for:
  - System status and layer scores
  - Pulse log browsing
  - Self-model and identity snapshots
  - Knowledge graph data
  - Health monitoring and crisis management
...
@app.get("/api/status")
def get_status():
    """System baseline: version, layer scores, success rate."""
...
@app.get("/api/layers")
def get_layers():
    """All layer scores with metric breakdowns."""
    return {
        "layers": {
            lid: {"score": ls.score, "metrics": ls.metrics}
            for lid, ls in self_model.layer_scores.items()
...
def get_self_model():
    """Full self-model including self-concept and value axioms."""
...
def set_narrative(narrative: str):
    """Update the system's current narrative."""
...
def get_value_axioms():
    """Current value axiom states and weights."""
    return {
...
def reinforce_axiom(axiom_name: str, count: int = 1):
...
@app.get("/api/health")
def get_health():
    """Health check with crisis detection."""
    report = crisis_monitor.check_health(self_model)
    return report


@app.post("/api/crisis/resolve")
def resolve_crisis(resolution: str = "Manual resolution"):
    """Resolve an active crisis state."""
    return crisis_monitor.resolve_crisis(resolution)


@app.get("/api/transitions")
def get_transitions():
    """Recent state machine transitions."""
    from state_machine import StateMachine
    sm_path = ".recovery_state.json"
    # Transitions are tracked in state machine instances
    return {"transitions": []}


@app.get("/api/metrics/timeline")
def get_metrics_timeline():
    """Layer score timeline from snapshots."""
...
def get_l3_signals(limit: int = 20):
    """Recent signals from the L3 signal watcher."""
...
def poll_signals():
    """Force a signal watcher poll."""
    from identity.self_model import SelfModel
    signals = _l3_watcher.poll()
    sm = SelfModel()
    # Update signal_coverage metric based on results
    if len(signals) > 0:
        sm.update_layer_metric("L3", "signal_coverage", min(100.0, 85.0 + len(signals)))
    return {"signals_detected": len(signals), "total": _l3_watcher.get_signal_count()}


@app.get("/api/l3/goals")
def get_l3_goals():
    """Active goals from the L3 goal generator."""
    from identity.self_model import SelfModel
    from identity.crisis_monitor import CrisisMonitor
    sm = SelfModel()
    cm = CrisisMonitor()
    state = {
        "crisis_active": cm.state.active,
        "layer_scores": {lid: sm.get_layer_score(lid) for lid in ["L1","L2","L3","L4","L5","L6"]},
        "metrics": {lid: ls.metrics for lid, ls in sm.layer_scores.items()},
    }
    _l3_generator.generate_from_state(state)
    active = _l3_generator.get_active_goals(20)
    return {
        "stats": _l3_generator.get_stats(),
...
def rate_rrp_ambiguity(session_id: str, req: RRPAmbiguityRequest):
    ...
    _rrp_persist.save(engine)
    return {"ambiguity": engine.state.ambiguity.to_dict(), "compact": encode_compact(engine.state)}
...
def add_rrp_decision(session_id: str, req: RRPDecisionRequest):
    ...
    _rrp_persist.save(engine)
    return {"decisions": len(engine.state.decisions), "compact": encode_compact(engine.state)}
